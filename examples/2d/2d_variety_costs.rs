//! Creates a 30x30 world based on a heightmap that contains a variety of
//! costs, pixels with differing shades of grey. A white pixel is the most
//! efficient, a black pixel is impassable and the greys between represent good
//! and bad costs in the CostFields. The purpose of this is to show actors
//! preferring routes around the bad costs
	...
	for (sector_id, costfield) in fields.iter() {
		...
		for (i, cost) in costfield.get().iter().enumerate() {
			...
			if *cost == 255 {
				...
				let grey_scale = (255.0 - *cost as f32) / 255.0;
				cmds.spawn((
					Sprite {
