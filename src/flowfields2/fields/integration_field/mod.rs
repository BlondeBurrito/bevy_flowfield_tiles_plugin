...
impl IntegrationField {
	...
	/// From the goal of the target sector calculate LOS
	...
	//TODO: diamond like propagation and wasted extra lookups looking at previously calculated neighbours, try fast marching method of solving Eikonal PDE for a spherical approx that visits each cell once
...
//TODO how would portals work with a goal
				...
				|| cost & INT_BITS_GOAL == INT_BITS_GOAL
			{
				...
				match dir {
					Ordinal::North | Ordinal::South => {
						// check if the corner is actually reachable from the neighbouring wavefront cell
						// this prevents stepping between two diagonal impassable cells
						// and assigning an incorrect wavefront cost to a corner that shouldn't exist
