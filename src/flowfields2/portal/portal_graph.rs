...
impl PortalGraph {
	...
	fn astar(
		...
		if !nodes.contains(&source_node) {
			error!(
				"Node data does not contain start node {:?}, this is probably a bug, please report it",
				source_node
			);
		...
		if !nodes.contains(&target_node) {
			error!(
				"Node data does not contain end node {:?}, this is probably a bug, please report it",
				target_node
			);
