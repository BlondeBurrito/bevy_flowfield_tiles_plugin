...
use crate::flowfields2::{
	...
	sectors::{MapDimensions, SectorID},
...
impl SectorCostFields {
	...
	pub fn new(map_dimensions: &MapDimensions) -> Self {
		...
		let column_count = map_dimensions.get_length()
			/ (map_dimensions.get_unit_scale() * FIELD_RESOLUTION as f32);
		let row_count = map_dimensions.get_depth()
			/ (map_dimensions.get_unit_scale() * FIELD_RESOLUTION as f32);
	...
	fn scale_costfield(&mut self, sector_id: &SectorID, map_dimensions: &MapDimensions) {
		...
		if map_dimensions.get_actor_scale() == 1 {
			...
			for (index, cost) in field_array.iter().enumerate() {
				if *cost == 255 {
					impassable_indices.push(index);
				}
			}
			...
			for index in impassable_indices.iter() {
				let this_cell = FieldCell::from_index(*index);
				// North
				let mut processed: BTreeMap<SectorID, Vec<FieldCell>> = BTreeMap::new();
				'ord: for i in 1..=map_dimensions.get_actor_scale() as usize {
					if let Some(n_row) = this_cell.get_row().checked_sub(i) {
						let field_cell = FieldCell::new(this_cell.get_column(), n_row);
						...
						let neighbours = {
							let mut n = Vec::new();
							//TODO does this just always run a single time? it never explores neighbours?
							for factor in
								0..=(map_dimensions.get_actor_scale() / FIELD_RESOLUTION as u32)
							{
								if let Some(n_sector) = map_dimensions.get_sector_id_from_ordinal(
									...
									&SectorID::new(
										...
										sector_id.get_row() - factor as i32,
									),
								) {
									n.push(n_sector);
								}
						...
						for (count, n_sector) in neighbours.iter().enumerate() {
							// adjust sizing to step through neighbour sector
				...
				// East
				'ord: for i in 1..=map_dimensions.get_actor_scale() as usize {
					if this_cell.get_column() + i < FIELD_RESOLUTION {
						let field_cell =
							FieldCell::new(this_cell.get_column() + i, this_cell.get_row());
						update_processed(&mut processed, field_cell, sector_id);
						let value = self
							.get_baseline()
							.get(sector_id)
						...
						let neighbours = {
							let mut n = Vec::new();
							//TODO Again does this do anything??
							for factor in
								0..=(map_dimensions.get_actor_scale() / FIELD_RESOLUTION as u32)
							{
								if let Some(n_sector) = map_dimensions.get_sector_id_from_ordinal(
									Ordinal::East,
									&SectorID::new(
										sector_id.get_column() + factor as i32,
										sector_id.get_row(),
									),
								) {
									n.push(n_sector);
						...
						for (count, n_sector) in neighbours.iter().enumerate() {
								...
								- i - (count * FIELD_RESOLUTION)
							{
								if x < FIELD_RESOLUTION {
									let field_cell = FieldCell::new(x, this_cell.get_row());
									update_processed(&mut processed, field_cell, n_sector);
									let value = self
										.get_baseline()
										.get(n_sector)
				...
				// South
				'ord: for i in 1..=map_dimensions.get_actor_scale() as usize {
					if this_cell.get_row() + i < FIELD_RESOLUTION {
						let field_cell =
							FieldCell::new(this_cell.get_column(), this_cell.get_row() + i);
						update_processed(&mut processed, field_cell, sector_id);
						let value = self
							.get_baseline()
							.get(sector_id)
						...
						let neighbours = {
							let mut n = Vec::new();
							//TODO again not sure if this does anything
							for factor in
								0..=(map_dimensions.get_actor_scale() / FIELD_RESOLUTION as u32)
							{
								if let Some(n_sector) = map_dimensions.get_sector_id_from_ordinal(
						...
						for (count, n_sector) in neighbours.iter().enumerate() {
							...
							{
								if x < FIELD_RESOLUTION {
									let field_cell = FieldCell::new(this_cell.get_column(), x);
									update_processed(&mut processed, field_cell, n_sector);
									let value = self
										.get_baseline()
										.get(n_sector)
				...
				// West
				'ord: for i in 1..=map_dimensions.get_actor_scale() as usize {
					if let Some(n_column) = this_cell.get_column().checked_sub(i) {
						let field_cell = FieldCell::new(n_column, this_cell.get_row());
						update_processed(&mut processed, field_cell, sector_id);
						let value = self
							.get_baseline()
							.get(sector_id)
						...
						let neighbours = {
							let mut n = Vec::new();
							//TODO again might not be doing anything
							for factor in
								0..=(map_dimensions.get_actor_scale() / FIELD_RESOLUTION as u32)
							{
								if let Some(n_sector) = map_dimensions.get_sector_id_from_ordinal(
	...
	pub fn from_heightmap(map_dimensions: &MapDimensions, path: String) -> Self {
		...
		let hori_sector_count = (map_dimensions.get_length()
			/ (map_dimensions.get_unit_scale() * FIELD_RESOLUTION as f32))
			as u32;
		...
		let vert_sector_count = (map_dimensions.get_depth()
			/ (map_dimensions.get_unit_scale() * FIELD_RESOLUTION as f32))
			as u32;
	...
	pub fn from_bevy_2d_meshes(
		...
		let field_cell_unit_size = map_dimensions.get_unit_scale();
...
/// From a triple floating point representation of a mesh retrieve a list of the edges as index pairs
