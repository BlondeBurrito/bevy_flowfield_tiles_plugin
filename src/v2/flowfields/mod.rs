...
//! * FlowField - a 2D array built from the `IntegrationField` which describes how an actor should move (flow) across the world
...
//! ## Portal Graph
//!
//! For finding a path from one Sector to another at a Portal level all Sector Portals are recorded within a data structure known as `PortalGraph`. The Portals are stored as Nodes and Edges are created between them to represent traversable paths, it gets built in three stages:
//!
